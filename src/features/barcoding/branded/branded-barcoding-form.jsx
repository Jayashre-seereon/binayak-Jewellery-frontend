...
export default function BrandedForm({ open, setOpen, onSave }) {
  ...
  const addRow = () => {
    setItems([...items, { grossWt: 0, stoneWt: 0 }]);
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="!w-[95vw] !max-w-[1300px] h-[90vh] p-0 overflow-hidden">
        ...
        <DialogHeader className="px-6 py-4 border-b">
          <DialogTitle>Branded Ornament Barcoding</DialogTitle>
        </DialogHeader>
        ...
        <div className="p-6 overflow-y-auto h-[calc(90vh-130px)] space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4 border p-4 rounded">
            ...
            <div>
              <label className="text-xs">Certification Info</label>
              <Input className="mt-1 h-9" />
            </div>
            <div>
                ...
                <tbody>
                  {items.map((row, i) => (
                    <tr key={i} className="bg-white shadow-sm">
                      ...
                      <td><Input className="h-8 w-20" onChange={(e)=>updateItem(i,"grossWt",e.target.value)} /></td>
                      <td><Input className="h-8 w-20" onChange={(e)=>updateItem(i,"stoneWt",e.target.value)} /></td>
